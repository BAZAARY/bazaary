            ...
            <div id={`Producto ${index}`} className="rounded-lg shadow-xl w-full h-52 mb-5 bg-[#d9d9d9] md:h-60 md:rounded-sm">
                {urlimagen && nombreitem && precioitem ? (               
                    <div className="grid grid-rows-4 grid-cols-3 grid-flow-row h-full"> 
                        <img alt={`Imagen producto ${index}`} src={urlimagen.url} className="row-start-1 row-end-4 self-center justify-self-center ml-4 rounded-2xl h-4/5 w-11/12 max-w-[14rem]"/>
                        ...
                        <h2 className=" overflow-hidden whitespace-nowrap text-ellipsis row-start-1 col-span-2 text-xl text-center mt-4 self-start font-text ml-4 "> {nombreitem.nombre} </h2>
                        <h2 className=" self-center text-center justify-self-center row-start-2 col-span-2 font-text text-lg">Precio COP: ${precioitem.precio}</h2>
                    ...
                    <div className="flex h-full justify-center items-center"> 
                            <svg aria-hidden="true" className="w-10 h-10 mr-2 text-gray-200 animate-spin dark:text-gray-600 fill-[#FD9200]" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
